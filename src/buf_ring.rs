...
use crate::{
    common::*, read_buf::KernelBuffer, sys as libc, BufferGroupID, PhantomUnsend, BUF_SIZE,
    MAX_BUFFER_GROUP, RING_POOL_SIZE,
};

pub type SharedBufRingPool = Rc<RefCell<BufRingPool>>;

/// BufRingPool is not trivially copiable / clonable because IT IS STATEFUL
pub struct BufRingPool {
    pub(crate) buf_ring_ptrs: [*mut libc::io_uring_buf_ring; MAX_BUFFER_GROUP],
}

impl BufRingPool {
    pub fn into_shared(self) -> SharedBufRingPool {
    ...
    // FIXME: this is such a bloated impl
    pub unsafe fn get_read_buf(
        ...
        KernelBuffer {
            ...
            _market_unsend: Default::default(),
        }
    }
